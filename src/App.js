...
class App extends Component {
  ...
  getItem = Id => {
    ...
    this.setState(prevState => ({
      countriesList: [...prevState.countriesList, filteredList],
    }))
  }
  ...
  render() {
    const {countriesList} = this.state
    const filteredCountries = this.getCountriesList()

    return (
      <div className="app-container">
        <h1 className="title">Countries</h1>
        <ul className="countries-names-list">
          {countriesList.map(eachCountry => (
            <CountryList
              key={eachCountry.id}
              countryNamesDetails={eachCountry}
              clickOnCountry={this.clickOnCountry}
            />
          ))}
        </ul>
        <h1 className="visitedCountries">Visited Countries</h1>
        <>
